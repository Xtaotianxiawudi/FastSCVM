...
def evaluate_segmentation_results(segmentation_folder, gt_folder, output_path=None, export_table=False,
    ...
    if not (output_path is None) and (export_table):
        ...
        table_filename = path.join(output_path, 'RIM-ONE-r3-results.csv')
        # save the table
        save_csv_segmentation_table('/root/data2/LightM_UNet-Gloau-bd-end_original/results/1.csv', image_filenames, cup_dices, disc_dices, ae_cdrs, seg_cdrs)
    ...
    #                                                                        str(mae_cdr)))
    print('AE-CDR = {}'.format(str(mae_cdr)))
    # save the mean values in the output path
    if not (output_path is None):
        # initialize the output filename
        output_filename = path.join(output_path, 'n_evaluation_segmentation.csv')
        # save the results
        save_csv_mean_segmentation_performance('/root/data2/LightM_UNet-Gloau-bd-end_original/results/2.csv', mean_cup_dice, mean_disc_dice, mae_cdr, mean_cdr)
