...
def draw_ent(prediction, save_root, name):
    ...
    # --------------------------------------------------------------
    # print(save_root)  ./results/ent/gdrishtiGS
    # print(name.split('\\')[-1])   gdrishtiGS_001.png
    # print(os.path.join(save_root, 'disc', name.split('\\')[-1]))  ./results/ent/gdrishtiGS\disc\gdrishtiGS_001.png
    # --------------------------------------------------------------
    cv2.imwrite(os.path.join(save_root, 'disc', name.split('\\')[-1]) + '.png', disc)
    cup = construct_color_img(cup_ent)
    cv2.imwrite(os.path.join(save_root, 'cup', name.split('\\')[-1]) + '.png', cup)
...
def save_per_img(patch_image, data_save_path, img_name, prob_map, boundary_map, ROI_mask):
    ...
    H, W = ROI_mask.shape
    ROI_mask_rgb = np.zeros((H, W, 3), dtype=np.uint8)

    # 背景 → 白色
    ROI_mask_rgb[ROI_mask == 255] = [200, 218, 79]

    # cup 或 disc 区域 → 绿色
    ROI_mask_rgb[ROI_mask == 128] = [2, 2, 255]

    # cup + disc 重叠 → 红色
    ROI_mask_rgb[ROI_mask == 0] = [255, 22, 22]
    # save mask output
    Image.fromarray(ROI_mask_rgb).save(path3)
